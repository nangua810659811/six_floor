...
public class UserMetaControllerTestCase {
	...
	private final String LOGIN_URL = "http://localhost:8080/unifyum/usermanagement.req?action=login";

	private final String LOGOUT_URL = "http://localhost:8080/unifyum/usermanagement.req?action=logout";

	private final String CHECKLOGIN_URL = "http://localhost:8080/unifyum/usermanagement.req?action=checkLogin";
	...
	public void testUnRegister() {
		...
		// 再做注销
		String httpRet = HttpClientUtil
				.doPostRequest(UNREGISTER_URL, rs_params);
		JSONObject rs_ret = JSONObject.parseObject(httpRet);
		assertTrue(rs_ret.getBoolean("success"));
	}
}
